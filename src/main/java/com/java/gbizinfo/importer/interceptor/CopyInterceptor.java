...
public class CopyInterceptor implements Interceptor {
    ...
    private static final String COMPANY_COPY = """
            COPY company (
                ...
                qualification_grade, update_date, inserted_at
    ...
    private static final String PATENT_COPY = "COPY patent (patent_id, company_id, patent_type, registration_number, application_date, title, url, update_date, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String CLASSIFICATION_COPY = "COPY classification (patent_id, code_value, code_name, japanese, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String CERTIFICATION_COPY = "COPY certification (company_id, date_of_approval, title, target, government_departments, category, update_date, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String SUBSIDY_COPY = "COPY subsidy (company_id, date_of_approval, title, amount, target, government_departments, update_date, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String WORKPLACE_INFO_COPY = "COPY workplace_info (workplace_info_id, company_id, update_date, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String BASE_INFO_COPY = "COPY base_info (workplace_info_id, average_continuous_service_years_type, average_continuous_service_years_male, average_continuous_service_years_female, average_continuous_service_years, average_age, month_average_predetermined_overtime_hours, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String WOMEN_ACTIVITY_INFO_COPY = "COPY women_activity_info (workplace_info_id, female_workers_proportion_type, female_workers_proportion, female_share_of_manager, gender_total_of_manager, female_share_of_officers, gender_total_of_officers, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String COMPAT_CHILDCARE_COPY = "COPY compatibility_of_childcare_and_work (workplace_info_id, number_of_paternity_leave, number_of_maternity_leave, paternity_leave_acquisition_num, maternity_leave_acquisition_num, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String COMMENDATION_COPY = "COPY commendation (company_id, date_of_commendation, title, target, category, government_departments, note, update_date, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String PROCUREMENT_COPY = "COPY procurement (company_id, date_of_order, title, amount, government_departments, note, update_date, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String FINANCE_COPY = "COPY finance (finance_id, company_id, accounting_standards, fiscal_year_cover_page, update_date, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String MAJOR_SHAREHOLDER_COPY = "COPY major_shareholder (finance_id, name_major_stakeholders, shareholding_ratio, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
    private static final String MANAGEMENT_INDEX_COPY = "COPY management_index (finance_id, period, net_sales_summary_of_business_results, net_sales_summary_of_business_results_unit_ref, operating_revenue1_summary_of_business_results, operating_revenue1_summary_of_business_results_unit_ref, operating_revenue2_summary_of_business_results, operating_revenue2_summary_of_business_results_unit_ref, gross_operating_revenue_summary_of_business_results, gross_operating_revenue_summary_of_business_results_unit_ref, ordinary_income_summary_of_business_results, ordinary_income_summary_of_business_results_unit_ref, net_premiums_written_summary_of_business_results_ins, net_premiums_written_summary_of_business_results_ins_unit_ref, ordinary_income_loss_summary_of_business_results, ordinary_income_loss_summary_of_business_results_unit_ref, net_income_loss_summary_of_business_results, net_income_loss_summary_of_business_results_unit_ref, capital_stock_summary_of_business_results, capital_stock_summary_of_business_results_unit_ref, net_assets_summary_of_business_results, net_assets_summary_of_business_results_unit_ref, total_assets_summary_of_business_results, total_assets_summary_of_business_results_unit_ref, number_of_employees, number_of_employees_unit_ref, inserted_at) FROM STDIN WITH (FORMAT csv, NULL '')";
